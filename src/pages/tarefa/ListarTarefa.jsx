    ...
    <>
      <Card>
        <CardHeader
          ...
          subheader="Procure uma tarefa pelo título ou descrição"
        />
        <Search/>
        <TextField onChange={(event) => setSearchTerm(event.target.value)} />
      </Card>
            ...
            >
              ...
              <TableBody>
                {tarefas.map((row, indice) => {
                  if (searchTerm.trim() !== "") {
                    if (!row.tituloTarefa.toLowerCase().includes(searchTerm.toLowerCase()) && !row.descricaoTarefa.toLowerCase().includes(searchTerm.toLowerCase())) {
                      return <></>
                    }
                  }
